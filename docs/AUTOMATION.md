...
| 4c | Breadth: the loop reaches the eight unreported cases on its own cadence; Deep Memory from the Birdmen inputs, from scratch | Running (weekly sittings since 2026-09-09). **Deep Memory opened 2026-09-09**: a question-only case (`content/cases/deep-memory/`, an opening edition with no assessment, an empty ledger, no inputs yet). The founder's pages enter by his own drop from GitHub (the founder-drop door; the panel's GPT seat on #236 read §3.15 as requiring the founder's own grant for republished copies, so the operator commits none); the intake then registers them as founding inputs, and the scheduler's next choices are the draft from that intake, verify, the first assessing edition, and the blind check — the loop's, unattended. Until the drop, the case is the least recently reported and gets a research pass at its turn. |
