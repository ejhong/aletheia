...
The vasocomputation sitting (#372), answered twice, carried three editions and three assessments; the diff budget dropped evidence.yaml and every assessment, and the run account itself, built in file order, ran out at the head edition's first line — three seats could only say unsure, and two unsures park. The account is now assembled by importance: the head edition (its article whole) and the assessment it adopts (every claim's verdict) first; then a digest of the records the change adds to evidence, claims, sources and research — id, state, direction, statement, quote and locator, read from the head file by the ids the diff adds; then the run records and the lines added to history and dispositions; then any candidate superseded within the change by the one naming it as previous, and any assessment such a candidate adopted, by what they say that the head does not — the paragraphs of the article the head does not carry verbatim, the claim verdicts that differ from the head's, each beside the head's — since a superseded edition is published in git even though the site renders only the head. Over the cap, whole sections are dropped from the least important and named; the head edition and the assessment it adopts are never dropped and the account is never cut — if they alone exceed the cap, the account runs over it and says so. (Corrected in place before publication, the same day, on review note #377: the GPT seat found that the first drop loop could take the head once nothing else remained, that a closing clip cut whatever was left, and that the superseded candidates had been reduced to a header alone. Building #372's account under the corrected loop then showed the history and dispositions lines falling before three verification files, so the sections of a rank now fall largest first, verification.md stands as its own section so it can fall alone, a superseded assessment's small digest falls with the run records rather than before them, and the cap is 200,000: #372's material runs to about 213,000 characters, and at 150,000 the verifier's own account had to go.) Supervised-by: founder (direction in session, 2026-09-20).
