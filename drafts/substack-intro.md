# Aletheia: An Evidence Atlas for Contested Ideas

I keep getting pulled toward a particular kind of idea: too interesting to ignore, too contested to trust. Quantum collapse in the brain. Muscle knots as stored autonomic state. Star-like flashes photographed seven years before Sputnik. Every time I go digging, I find the same landscape: two literatures that barely speak to each other. Proponents cite proponents, skeptics cite skeptics, and the thing I actually want — which observations are real, which have been independently replicated, and what experiment would settle it — is written down nowhere.

The usual way to engage with ideas like these is to pick a tribe. I would rather have a map.

So I built one. It's called **Aletheia**, it's live at [ejhong.github.io/aletheia](https://ejhong.github.io/aletheia/), and this post is the tour.

## What it is

Each case opens as an ordinary article, the kind of piece I'd post here. But the load-bearing sentences link down into a ledger, and a few design decisions do most of the work:

**Claims are atomic.** One proposition, one truth condition. "Microtubules do interesting quantum things" is not a claim; "excitation travels ~6.6 nm through microtubules, further than standard theory predicts" is. Splitting the rungs stops evidence for one rung from silently counting for every rung above it.

**Evidence has a declared direction.** Every evidence record says whether it supports, undermines, qualifies, or merely contextualizes a claim. Undermining evidence gets the same shelf space as supporting.

**Credibility and diagnosticity are kept apart.** How likely is this observation to be true — and separately, if true, how much does it favor this hypothesis over the boring alternative? A perfectly credible observation can be nearly worthless as evidence for a grand theory. Blurring those two numbers is how most bad arguments about anomalies work.

**The AI involvement is labeled, not hidden.** AI agents did much of the extraction and drafting, and every record says so: claims are stamped `ai_extracted`, citations carry verification labels, and AI-drafted assessments are marked as drafts, not passed off as reviewed human conclusions. A human (me) reviews and signs off, and the review itself goes in the record.

**Everything is versioned.** When an assessment changes, the history shows what changed and why. A weekly literature watch runs case-specific queries against arXiv and Crossref — including queries built to find the *critical* literature — and nothing enters the record without review.

The point is not to issue verdicts. It's to make the disagreement precise enough that you can see exactly where it lives.
...
The strangest case on the site. Walter von Lucadou's Model of Pragmatic Information says psi effects are real but can never carry a signal, so any experiment rigorous enough to exploit them must watch them decline or scatter — it predicted its own replication failures, in print, before they happened. In 2019 the gravitational-wave physicist Hartmut Grote unblinded a 200-participant, fully blind-analyzed test live on stage: null, which is what the model predicts a well-built experiment will show, and also exactly what nothing-at-all looks like. The open question: can the model's proponents state binding predictions for the decline's fine structure in advance, and do those survive adversarial replication?

## Two more on the bench

**Zero Worlds** maps Markus Müller's observer-first physics — the wager that you can start from zero external worlds, just observers and algorithmic probability, and get lawful physics out as a theorem (regular readers will remember my July piece). And **Conformal Cyclic Cosmology** takes up Penrose's proposal that the universe's far future is the big bang of the next aeon, with claimed observational traces that others cannot find. Both get the same treatment: rungs separated, evidence direction declared, disconfirmers up front.
