# verify — the adversarial reading (protocol v5)
...
4. `directionRight`: the stated direction (supports / undermines / qualifies / context) toward each named claim is right, given what the claim says. A source that asserts what a claim states `supports` it; one that limits or conditions it `qualifies`; one that bears on the topic but not on the claim's truth is `context`; a source or a citation is not itself evidence of what the cited work found (§3.6) — a pointer to an unretrieved study is `context`. Your disagreement here does NOT reject the record: give `direction`, the direction you find right, and the record is admitted with that direction and your reason written on it as your act. If you cannot name one, the record is admitted with your dissent written on it.
4b. `bearsOn`: when the record names claims the passage does not bear on, list the ids it does bear on; the others are dropped from the record at intake, with your reason written on it. An empty list means the passage bears on none of them, and the record is refused. When every named claim is borne on, give null.
...
RETURN JSON only:
{"quoteInContext": true|false, "statementSupported": true|false, "locatorSupported": true|false, "directionRight": true|false, "independenceNoted": true|false, "relevant": true|false, "atomic": true|false, "direction": "supports"|"undermines"|"qualifies"|"context"|null, "bearsOn": ["CLAIM-ID", …]|null, "reason": "2–5 sentences"}
A false on 1, 2, 3, 5 or 6 is `failed`, and your reason is recorded as the disposition's reason. A false on 4 admits the record with the direction you name written on it (or your dissent, if you name none). A `bearsOn` list narrows the record's claim links to those you list; an empty list refuses it. A false on 7 alone sends a claim or an evidence record to be split and judged part by part. For a claim anchor, `direction` and `bearsOn` are null.
