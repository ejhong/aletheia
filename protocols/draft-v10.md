# draft — from a report to a proposal (protocol v10)
...
v10 (2026-09-20): a report may be a leads pass — works an earlier pass
named and could not open, asked of the open indexes: what it lists as
"Resolved" agreed with the lead on the checks named, what it lists as
"Candidates" may be the work; each lead carries its disposition row's
`key`, and the row you write for it carries that key back, so a lead's
whole story sits under one row.
...
v7 (2026-09-10): a research item proposes a test and names the claims it
would move; it states no result or figure from a source the ledger does
not hold. The first third-party drop (a post naming two preprints) had
every record from the preprints refused for a retrieval fault, and the one
research item that survived carried a figure from the refused text into the
public agenda with no source behind it (the Arbiter's Opus and GPT seats on
#266). Findings enter as evidence records, or not at all.
...
- When the report is an inbox intake, the works it lists as "resolved" are CANDIDATES found by title or by author and year, each with its similarity score: confirm from the retrieved text (its title, authors, and content) that it is the work the supplied text meant before anchoring anything to it.
- When the report is a leads pass ("Leads reopened"), each lead is a work an earlier pass named and could not open, quoted exactly as its disposition row records it, with that row's `key`. A document under "Resolved" agreed with the lead on the checks named beside it; a document under "Candidates" may be the work — confirm from the retrieved text before anchoring anything, and disposition a wrong one `failed` ("resolved to the wrong work: …"). Write one disposition row per lead and put the lead's `key` in the row's `key` field, exactly as given: `blocked` again, with the route, when nothing opened; the verifier writes the `in` row when a source you propose for the lead enters. A wrong candidate is dispositioned `failed` ("resolved to the wrong work: …") and the work the text meant stays `blocked` with the route to find it. The supplied text itself is its supplier's words, never a source.
...
- `adds.research`: a decisive test or archival route worth doing, with the claims it would move and what it would settle. A research item describes the test — what would be measured, on what, and what result would move which claim, in both directions where the test can come out either way; it does not report a result or a figure from any source, and never from one that is not in the ledger: a finding belongs in an evidence record, with its quote, or nowhere. When the report sharpens a test the ledger already proposes — a better analyte, a page number, a control — write a `corrections` entry on that item's `summary` or `informationGain` rather than a new item; a new item is for a test the ledger does not yet propose. The verifier refuses an item that names no measurement, no object, or no decision rule.
...
## Two rules the first Orch OR sitting taught (v8, 2026-09-11)

- **Refer to the records you propose by title, never by an id.** Your
  numbering never enters the ledger: the verb keys each proposed record
  from the case's next free id, and verification drops what it rejects,
  so an id you invent (ORCH-C101, "see C3") names nothing once published.
  Existing ledger records are cited by their ids; a proposed claim, source,
  evidence record or research item is named in prose by its title or its
  proposition, and the verb rejects any proposed record whose prose names
  an id the ledger does not hold.
- **A claim is one proposition.** Do not append a falsification condition
  to a statement ("…; it would be false if …", "…, falsifiable by …"). The
  condition is a second, independently testable proposition (§3.2); the
  verb rejects the statement. What would falsify a claim belongs in the
  research item that would test it, or in the evidence that bears on it.
