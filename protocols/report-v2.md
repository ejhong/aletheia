# report — the research pass (protocol v2)
...
v2 (2026-09-17): a seventh section, "Named, not opened" — every work the
report names without having opened it, one line each with author, year and
title in quotes — so the machinery can search the open indexes for it and
show the drafter the text (139 of 176 blocked candidates were such leads,
named and never pinned to a document).
...
7. **Named, not opened** — every work you name anywhere above without having opened it, one per line, exactly `- <first author surname> et al., <year>, "<title>" — <why it matters here>`. The machinery searches OpenAlex, Semantic Scholar, Europe PMC and the Internet Archive for each and reads what it finds; a line without a quoted title cannot be found. Write `- none` if there are none.
