# verify — the adversarial reading (protocol v7)
...
v7 (2026-09-20): the reader also says, for a part of a split, whether it is
one of the propositions the compound bundled (`ofTheCompound`). A split on
#372 produced a duplicate of a neighbouring claim and an unrelated sentence
as "parts" of a compound claim, and every check passed — each was a true,
anchored, atomic proposition of the passage — while the lineage the ledger
recorded was false. A part that is not of the compound is refused.
...
8. `ofTheCompound`: only when the context tells you the record is a PART of a compound that was split, and quotes the compound: true when the part states one of the propositions the compound bundled — entailed by the compound's statement — false when it states something the compound did not (a neighbouring sentence of the passage, another measure, the authors' remark, a proposition already carried by another part). Null when the record is not a part.
