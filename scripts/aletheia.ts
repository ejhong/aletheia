...
switch (verb) {
  ...
  case "next": {
    ...
    const forcedVerb = (flagValue("--verb") ?? "report") as "report" | "edition" | "check" | "reverify" | "leads";
    if (forcedCase && !["report", "edition", "check", "reverify", "leads"].includes(forcedVerb)) { console.error(`--verb must be report, edition, check, reverify or leads (got ${forcedVerb})`); process.exit(2); }
  ...
  case "leads": {
    const [key] = args;
    if (!key) {
      console.error("usage: aletheia leads <case> [--dry-run]");
      process.exit(1);
    }
    const r = await runLeads(key, { dryRun: flags.has("--dry-run") });
    console.log(JSON.stringify(r, null, 2));
    if (r.outcome === "failed") process.exit(1);
    break;
  }
