...
const packet = [
  ...
  omitted.length > 0
    ? `=== OMITTED FILES (diff over size budget — you have NOT seen these in the DIFF; a file marked as read into the RUN ACCOUNT is shown there, as its section says; the rest carry a mechanical shape — size, and for JSON its fields, with identifiers whole so you can match them against the RUN ACCOUNT — generated by this repository's tooling from the committed bytes) ===\n${omittedNotes(omitted, account.files, (p) => git("show", `${head}:${p}`)).join("\n")}`
...
const report = [
  ...
  "",
  omitted.length > 0
    ? `> ⚠️ ${omitted.length} file(s) exceeded the diff budget and were not shown to the panel in the diff (${omitted.filter((f) => account.files.includes(f)).length} of them read into the run account): ${omitted.join(", ")}`
    : "",
