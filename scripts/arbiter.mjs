...
const packet = [
  ...
  omitted.length > 0
    ? `=== OMITTED FILES (diff over size budget — you have NOT seen these in the DIFF; a file marked as read into the RUN ACCOUNT is shown there, as its section says; the rest carry a mechanical shape generated by this repository's tooling from the committed bytes: the file's size, and for JSON its fields, where ${shapeRule()}) ===\n${omittedNotes(omitted, account.files, (p) => git("show", `${head}:${p}`)).join("\n")}`
