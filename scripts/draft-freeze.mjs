...
import { applyBudgets } from "./lib/bench-core.mjs";
 ...
 * advancing study proposals from the merged intake score history whose freeze
...
function scanAdvancing() {
  ...
  return readAgendaTallies(ROOT).filter(
    (proposal) => proposal.advances && !draftedIds.has(proposal.id),
  );
}
...
if (!advancing) {
  console.error(
    "usage: node scripts/draft-freeze.mjs <advancing.json | --scan>",
  );
  process.exit(1);
}
...
for (const p of advancing) {
  const dir = path.join(
    CASES,
    resolveCaseDirectory(ROOT, p.caseSlug),
    "studies",
  );
...
for (const p of selected) {
  ...
  const bad = [];
  if (!r?.title || !r?.summary || !r?.informationGain)
    bad.push("research item incomplete");
  if (
    !s?.title ||
    !s?.question ||
    !Array.isArray(s?.inclusion) ||
    s.inclusion.length < 2
  )
    bad.push("study criteria incomplete");
  if (!Array.isArray(s?.columns) || s.columns.length === 0)
    bad.push("no columns");
  if (typeof s?.method !== "string" || s.method.length < 40)
    bad.push("method too thin");
  ...
  const existingS = fs.existsSync(studiesDir)
    ? fs
        ...
        .map((f) => Number((f.match(/-s(\d+)\.yaml$/) ?? [])[1] ?? 0))
    : [];
      ...
      stringifyYaml([
        {
          ...
          title: r.title,
          summary: r.summary,
          claimIds: r.claimIds ?? s.claimIds,
          track: "publication_prize",
          effortTier: "desk",
  ...
  const study = {
    ...
    criteria: {
      ...
      knownCandidates: (s.knownCandidates ?? []).filter(
        (k) =>
          k?.name &&
          ["include", "exclude"].includes(k?.disposition) &&
          k?.reason,
      ),
  ...
  execFileSync("node", ["scripts/stamp-study.mjs", p.caseSlug, sid], {
    stdio: "inherit",
  });
  fs.appendFileSync(
      ...
      stringifyYaml([
        {
          ...
          reason:
            "Bench v2 (docs/AUTOMATION.md): proposals scored high-gain by four of five seats with no constitutional concern auto-draft their freeze; the panel judges the protocol blind to any data.",
