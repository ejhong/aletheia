...
import { readAgendaTallies } from "../../scripts/lib/intake-agenda.mjs";
...
export interface AgendaProposal {
  ...
  wouldSettle: string;
  effortTier: string;
  /** Bench panel score, from the shared intake history. */
  score?: ProposalScore;
...
export interface ProposalScore {
  ...
  seats: string[];
  /** Recorded explanations; absent when historical reasons were not retained. */
  review?: {
    seat: string;
...
export function parseProposalFile(
  ...
  const meta = {
    ...
    date:
      text.match(/^# Agenda proposals — .+ — (\d{4}-\d{2}-\d{2})/m)?.[1] ??
      "unknown",
...
export function loadProposalRuns(): ProposalRun[] {
  ...
  const scores = new Map<string, ProposalScore>(
    readAgendaTallies().map((tally) => [tally.id, tally]),
  );
  return fs
    ...
    .map((runId) => {
      ...
      const files = fs
        ...
        .map((f) => {
          ...
          pf.proposals = pf.proposals.map((p) => ({
            ...p,
            score: scores.get(`${runId}/${pf.caseSlug}/${p.index}`),
            draftedAs: drafted.get(`${runId}/${pf.caseSlug}/${p.index}`),
            adoptedAs: adopted.get(`${runId}/${pf.caseSlug}/${p.index}`),
          }));
          return pf;
        })
