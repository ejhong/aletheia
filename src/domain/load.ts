...
import { danglingProseRefs } from "./proseRefs.ts";
...
function checkIntegrity(caseDir: string, loaded: LoadedCase): void {
  ...
  // ids; the same day's repair pass re-pointed or rewrote 29 older references
  // so the rule could hold everywhere — review note #285).
  const dangling = danglingProseRefs(loaded);
  if (dangling.length) {
