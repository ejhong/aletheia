...
export async function runDraft(reportRunId: string, opts: DraftOptions = {}): Promise<DraftOutcome> {
  ...
  const leads: { lead: string; resolved: { title: string; identifier: string; url: string; via: string; agreed: string[] } | null }[] = [];
  for (const lead of leadsNamedInReport(report).slice(0, 12)) {
    const resolved = await (opts.deps?.resolveLead ?? resolveLead)(lead);
    leads.push({ lead: lead.text, resolved: resolved ? { title: resolved.title, identifier: resolved.identifier, url: resolved.url, via: resolved.via, agreed: resolved.agreed } : null });
